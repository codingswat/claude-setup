...
| [hooks/](hooks/) | The scripts that enforce the rules: the rules injector, the card injector, the dangerous-git blocker, the bare-commit guard, the heavy-test-suite guard, the auto-backup with a secret scan, a ledger that records what each helper agent cost — and a test script that proves each one fires and each one stays quiet. | — |
